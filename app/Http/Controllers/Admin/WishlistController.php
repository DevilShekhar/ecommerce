...
class WishlistController extends Controller
{
    ...
    public function filter(Request $request)
    {
        ...
        if ($request->has('rec_min_price') && ! empty($request->rec_min_price)) {
            $query->where('price', '>=', (float) $request->rec_min_price);
        }
        if ($request->has('rec_max_price') && ! empty($request->rec_max_price)) {
            $query->where('price', '<=', (float) $request->rec_max_price);
        }
                ...
                <div>
                    <div class="jewel-product-card product-details-trigger" data-product-id="'.$product->id.'">
                        <div class="jewel-product-image">
                            ...
                            '.($activeOffer ? '<span class="jewel-badge offer">'.($activeOffer->discount_type === 'percentage' ? rtrim(rtrim(number_format($activeOffer->discount_value, 2), '0'), '.').'% OFF' : '₹'.number_format($activeOffer->discount_value, 0).' OFF').'</span>' : '').'
                        ...
                        <div class="jewel-product-info">
                            ...
                            <div class="jewel-price-row">
                                '.($activeOffer ? '
                                    <span class="jewel-price">₹'.number_format($discountedPrice, 0).'</span>
                                    <span class="jewel-old-price">₹'.number_format($originalPrice, 0).'</span>
                                ' : '<span class="jewel-price">₹'.number_format($originalPrice, 0).'</span>').'
                            </div>
