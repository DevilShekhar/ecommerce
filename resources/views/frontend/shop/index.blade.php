...
<script>
    let currentPage = 1;
    let isLoading = false;
    let initialCategory = '{{ $filters["category"] ?? "" }}';
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // CHECK FOR CATEGORY SLUG FROM URL OR BACKEND
        const urlParams = new URLSearchParams(window.location.search);
        const categorySlugFromUrl = urlParams.get('category') || initialCategory;

        if (categorySlugFromUrl) {
            // Set the category dropdown value
            const categorySelect = document.getElementById('category');
            const optionExists = Array.from(categorySelect.options).some(option => option.value === categorySlugFromUrl);

            if (optionExists) {
                categorySelect.value = categorySlugFromUrl;
                // Load subcategories and products
                loadSubcategories(categorySlugFromUrl);
            } else {
                // If category doesn't exist in dropdown, just load products
                applyFilters();
            }
        } else {
            // Load subcategories if category is already selected in dropdown
        ...
        if (categorySlugFromUrl) {
            const breadcrumbSpan = document.getElementById('breadcrumbCategory');
            if (breadcrumbSpan && categorySlugFromUrl) {
                const categoryName = document.getElementById('category').options[
                    document.getElementById('category').selectedIndex
                ]?.text || categorySlugFromUrl;
                breadcrumbSpan.textContent = ` / ${categoryName}`;
            }
        }
    });
    ...
    function loadSubcategories(categorySlug) {
        ...
        if (!container || !subcategorySelect) {
            applyFilters();
            return;
        }
    ...
    function getFilters() {
        ...
        const subcategorySelect = document.getElementById('subcategory');
        if (subcategorySelect) {
            filters.sub_category = subcategorySelect.value;
        }
    ...
    // =============================================
    // RENDER PRODUCTS (WITH WISHLIST & CART BUTTONS)
    // =============================================
